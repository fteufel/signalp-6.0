...
def display_path_from_save_path(path: str):
    """Convert the absolute path to save a file to the relative path for server display"""
    html_path = path
...
def postprocess_probabilities(
    all_probs: np.ndarray, all_global_probs: np.ndarray, kingdom_id: str
):
    ...
    Sum all I,M,O into Sec/SPI I (=0)
    """
    all_probs_out = all_probs.copy()

    # set 0 in out, then fill in again.
    # TODO eukarya can be vectorized, no need for loop
    ...
    if kingdom_id == "Eukarya":
        ...
        # need stupid [:,:,-1] after sum, fancy indexing sum does not remove dim apparently
        ...
        all_global_labels = np.argmax(all_global_probs, axis=-1)
        for idx in range(len(all_global_labels)):
            if all_global_labels[idx] == 2:
                all_probs_out[idx, :, 10] = all_probs[idx, :, [10, 11]].sum(axis=-1)
                all_probs_out[idx, :, 11] = 0
    # WIP: sum all other states into Sec/SPI I (this is the first one)

    all_probs_out[:, :, LABEL_IDS_OTHER] = 0
...
def make_plots(
    ...
    for idx, identifier in enumerate(identifiers):
        ...
        fig = sequence_plot(
            ...
            viterbi_paths[idx],
            sequences[idx],
            title="SignalP 6.0 prediction: " + identifiers[idx],
        )
...
def add_sequences_to_summary(
    ...
    """Adds each predicted sequence to the JSON output. Paths are converted before addint to the JSON."""
    if kingdom == "Eukarya":
    ...
    for idx, identifier in enumerate(identifiers):
        seq_dict = {
            "Name": identifier,
            "Plot_eps": display_path_from_save_path(eps_save_paths[idx]),
            "Plot_png": display_path_from_save_path(png_save_paths[idx]),
            "Plot_txt": display_path_from_save_path(txt_save_paths[idx]),
...
def main():
    ...
    parser.add_argument(
        "--organism",
        type=str,
    )  # default='Eukarya', const='Eukarya', nargs='?', choices=['Eukarya', 'Archaea','Gram-positive bacteria', 'Gram-negative bacteria'])
    ...
    with open(os.path.join(args.output_dir, "output_before.dump"), "w+") as outf:
        json.dump(vars(args), outf, indent=2)
    ...
    # marginal_probs = postprocess_probabilities(marginal_probs, global_probs, args.organism)
    ...
    result_summary["CSV_FILE"] = display_path_from_save_path(tsv_output_file)
